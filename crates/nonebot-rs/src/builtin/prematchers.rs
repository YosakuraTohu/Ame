use crate::builtin::matcher::PreMatcher;
...
#[doc(hidden)]
fn option_command_start_(event: &mut MessageEvent, config: BotConfig) -> bool {
    command_start_(event, config);
    true
}
...
pub fn option_command_start() -> Arc<PreMatcher<MessageEvent>> {
    Arc::new(option_command_start_)
}
